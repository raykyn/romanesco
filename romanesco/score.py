...
def score(data: str, load_from: str, batch_size: int, **kwargs):
    ...
    inputs, targets, loss, _, _, _, _, init_state = define_computation_graph(vocab.size, batch_size)
    ...
    with tf.Session() as session:
        ...
        for x, y in reader.iterate(raw_data, batch_size, NUM_STEPS):
            l = session.run([loss], feed_dict={inputs: x, targets: y, init_state: _current_state})
            total_loss += l[0]
            total_iter += 1
